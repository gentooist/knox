 ...
 * Required:
 *
 *  - `key`     amazon/google api key
 *  - `secret`  amazon/google secret
...
var Client = module.exports = exports = function Client(options) {
  this.service = (options.service || 'amazon').toLowerCase();
  if (!options.key) throw new Error('aws/goog "key" required');
  if (!options.secret) throw new Error('aws/goog "secret" required');
  if (!options.bucket) throw new Error('aws/goog "bucket" required');

  if ('google' === this.service) {
    this.identifier = 'GOOG1';
    this.xcode = 'x-goog';
    this.endpoint = 'commondatastorage.googleapis.com'
    this.addlHeaders = { 'x-goog-api-version' : 1 };
  } else {
    this.identifier = 'AWS';
    this.xcode = 'x-amz';
...
Client.prototype.request = function(method, filename, headers){
  ...
  headers.Authorization = auth.authorization({
    ...
    , storageHeaders: auth.canonicalizeHeaders(headers)
  });
...
Client.prototype.put = function(filename, headers){
  headers = utils.merge(headers || {}, this.addlHeaders);

  var defaults = {};
  defaults['Expect'] = '100-continue';
  defaults[this.xcode + '-acl'] = 'public-read';

  headers = utils.merge(defaults, headers);
